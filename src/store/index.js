...
export const useAdminAgentStore = defineStore("spatial", () => {
  ...
  function setSubscriptionFeedBack(feedBack) {
    subscriptionFeedBack.value = feedBack;
  }
  function getAgentSystemFromHQ() {
    return agent.headquarters.substring(0, agent.headquarters.length - 3);
  }
  ...
  async function getAgentData() {
    try {
      let response = await fetch(`${FETCH_URL}systems/${getAgentSystemFromHQ()}`, options);
      if (response.ok) {
        const json = await response.json();
        agentSystem.value = json.data;
      }
      response = await fetch(
        `${FETCH_URL}systems/${getAgentSystemFromHQ()}/waypoints/${agent.headquarters}`,
        options
      );
      if (response.ok) {
        const json = await response.json();
        agentWayPoint.value = json.data;
      }
    } catch (error) {
      console.log(error);
    }
